...
def parse_star_parameters(line, star):
    ...
    **star** — объект звезды.
    ...
    params = line.replace("\n", "").replace("\r", "").split(" ")
    print(f"Star params received: {params}")
    if len(params) != 8:
        print(f"Incorrect STAR configuration: {params}. Skipping...")
    star.R = int(params[1])
    star.color = params[2]
...
def parse_planet_parameters(line, planet):
    ...
    **planet** — объект планеты.
    """
    params = line.replace("\n", "").replace("\r", "").split(" ")
    print(f"Planet params received: {params}")
    if len(params) != 8:
        print(f"Incorrect PLANET configuration: {params}. Skipping...")
